		...
		if (empty($chainField)){
			...
			if ($arProperty['PROPERTY_TYPE'] == 'L'){
				...
				$enumSql = "
					...
					WHERE PROPERTY_ID = '".$arProperty['ID']."'";

				$obEnums = $connect->query($enumSql);
				while($enum = $obEnums->fetch()){
					$enums[$enum['CODE']][$enum['ID']] = $enum['VALUE'];
				}
			}

			$cacheField->addFetchDataModifier(
				function ($value, $field, $data, $alias) use ($arProperty, $enums) {
...
//			Entity\Base::destroy($utmClassNameFull);
	...
	public static function cleanPropCache($iblockId)
    {
        $cacheId = self::getCacheId($iblockId);
        $tagCache = new Data\TaggedCache();
        $tagCache->clearByTag($cacheId);
    }

    protected static function getCacheId($iblockId)
    {
        return self::$cachePropId.$iblockId;
    }
	...
	public function metaData()
	{
		...
		$cacheId = self::getCacheId($iBlock);
		$cacheTime = self::$cachePropTime;
