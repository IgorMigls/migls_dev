...
class Debug extends \Bitrix\Main\Diag\Debug
{
	 ...
	 * @return mixed
	 */
	public static function getMemory($flag = false, $show = true)
	{
		...
		$formatMemory = \CFile::FormatSize($endMemory);
		if($show){
			PR($formatMemory);
		}
		return $formatMemory;
...
//		self::$memory = $start;
	...
	public static function toLog($sText, $pathLog = false, $traceDepth = 10, $bShowArgs = false){
		...
		if(!$pathLog){
			$dir = $root.'/local/lg';
			$pathLog = $dir.'/ls_lg97907623423.log';
		}

		$Dir = new IO\Directory($dir);
		$Dir->create();
